...
func TestFields_PtrToNonStructDoNothing(t *testing.T) {
	...
	field := result[0]
	testField(t, result[0], "Foo", "MY_APP", tag)

	rv := reflect.ValueOf(&config)
	testFieldReflect(t, rv, field, 0)
}

func testField(t *testing.T, f conf.Field, name, prefix string, tag conf.Tag) {
	t.Helper()
...
func testFieldReflect(t *testing.T, rv reflect.Value, field conf.Field, fieldNbr int) {
	...
	rvType := rv.Type()
	configName := rvType.Name()
	assert.Equal(t, configName, field.ParentStruct)
